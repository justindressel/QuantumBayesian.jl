...
"""
@inline function ham(dt::Time, H::QOp; ket=false)
...
@inline function ham(dt::Time, H::Function; ket=false)
...
"""
@inline function sham(dt::Time, H::QOp)
    const u::QOp = sparse(expm( -im * dt * full(H)))
    const l = superopl(u)*superopr(u')
    (t::Time, ρvec) -> l * ρvec
end
@inline function sham(dt::Time, H::Function)
...
"""
@inline function ham_rk4(dt::Time, H::Function; ket=false)
    inc(t::Time, ρ::QOp)::QOp = - im * comm(H(t),ρ)
    if ket
        inc(t::Time, ψ::QKet)::QKet = - im * H(t) * ψ
    end
    function rinc(t::Time, ρ)
        dρ1 = inc(t, ρ)
...
@inline function ham_rk4(dt::Time, H::QOp; ket=false)
    h(t) = H
...
"""
    lind_rk4(dt::Time, H::QOp, alist::QOp...)
...
time step `dt`. The triples in `mlist` have the form `(m, τ, η)`, where
`m` is a (generally non-Hermitian) operator specifying the measurement
and backaction (see below), `τ` is the timescale of the measurement 
collapse, and `η` is the measurement efficiency (bounded between 0 and 1).
...
The backaction takes the form of a three-step process:
  ...
  - the dephasing operators in `alist`
