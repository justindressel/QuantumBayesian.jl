...
@test h(0.0, q('x')) ≈ q('x')
h = ham(π/2, q('x'))
@test h(0.0, ground(q)) ≈ q(1,1)
h = ham(1e-2, 0*q('z'), ket=true)
@test h(0.0, xplus) ≈ xplus
h = ham(π/2, q('x'), ket=true)
@test h(0.0, groundvec(q)) ≈ -im*q(1)

# Test superoperator Hamiltonian increment
h = sham(1e-2, 0*q('z'))
@test h(0.0, superket(q('x'))) ≈ superket(q('x'))
h = sham(1e-2, t->0*q('z'))
@test h(0.0, superket(q('x'))) ≈ superket(q('x'))
h = sham(π/2, q('x'))
@test h(0.0, superket(ground(q))) ≈ superket(q(1,1))
...
@test unsuperket(l(0.0, superket(q(1,1))))[2,2] ≈ exp(-1e-6)
...
e = Ensemble(t[1])
@test e(1).t == range(0.0,stop=1.0,length=100)
